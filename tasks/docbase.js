...
module.exports = function(grunt) {
	...
	grunt.registerMultiTask('docbase', 'Grunt plugin to generate html files from your docbase project.', function() {
		...
		var generateSearchIndex = function(page, url, ph, buildIndex, callback) {
			...
			}, options.searchIndexSelector, url).then(function(elements) {
		...
		var generatePage = function(page, url, ph, callback) {
			...
			}, options.rootDocument).then(function(documentContent) {
		...
		var crawlPage = function(url, findLinks, versionFlag, callback) {
			...
			phantom.create([], {
				...
				page.setting('userAgent', 'Mozilla/5.0 (Windows NT 6.1; WOW64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/37.0.2062.120 Safari/537.36').then(function() {});
						...
						util.waitFor({
							...
							success: function() {
								...
								if (!options.onlysearchIndex) {
									...
									generatePage(page, url, phInstance, callback);
								} else {
									if (progressStart && !versionFlag) {
										generateSearchIndex(page, url, phInstance, true, callback);
									} else {
										if (callback) {
											callback(phInstance, url);
										}
									}
								}
